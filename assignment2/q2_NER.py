...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      embedding = tf.Variable(tf.random_uniform([len(self.wv), self.config.embed_size], -1, 1))
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('Softmax'):
      ...
      b2 = tf.get_variable('b2', (self.config.label_size,), initializer=xavier_weight_init())
      y_hat = tf.matmul(h, U) + b2

      if self.config.l2:
        tf.add_to_collection('total_loss', 0.5 * self.config.l2 * tf.nn.l2_loss(U))

      output = tf.nn.dropout(y_hat, self.dropout_placeholder)
    ### END YOUR CODE
    return output 

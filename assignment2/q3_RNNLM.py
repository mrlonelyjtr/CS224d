...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      embedding = tf.Variable(tf.random_uniform([len(self.vocab), self.config.embed_size], -1, 1), trainable=True)
      inputs = tf.nn.embedding_lookup(embedding, self.input_placeholder)
      inputs = [tf.squeeze(i, [1]) for i in tf.split(1, self.config.num_steps, inputs)]
      ### END YOUR CODE
      return inputs
  ...
  def add_loss_op(self, output):
    ...
    ### YOUR CODE HERE
    logits = [output]
    targets = [tf.to_int32(tf.reshape(self.labels_placeholder, [-1]))]
    weights = [tf.ones([self.config.batch_size * self.config.num_steps])]
    cross_entropy = sequence_loss(logits, targets, weights)
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN'):
      ...
      for t, i in enumerate(inputs):
        ...
        H = tf.get_variable('W', (self.config.hidden_size, self.config.hidden_size))
        I = tf.get_variable('I', (self.config.embed_size, self.config.hidden_size))
        b_1 = tf.get_variable('b_1', (self.config.hidden_size,))

        s = tf.nn.sigmoid(tf.matmul(s, H) + tf.matmul(i, I) + b_1)
        rnn_outputs.append(s)

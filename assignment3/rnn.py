...
class RNN_Model():
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            word = self.vocab.encode(node.word)
            curr_node_tensor = tf.expand_dims(tf.gather(embedding, word), 0)
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            children = tf.concat(1, [node_tensors[node.left], node_tensors[node.right]])
            curr_node_tensor = tf.nn.relu(tf.matmul(children, W1) + b1)
            ### END YOUR CODE
